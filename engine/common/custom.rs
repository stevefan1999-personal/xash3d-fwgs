...
*/
#[no_mangle]
pub unsafe extern "C" fn CustomDecal_Validate(mut raw: *mut libc::c_void,
                                              mut nFileSize: libc::c_int)
 -> qboolean {
    let mut test: *mut rgbdata_t =
        FS_LoadImage(b"#logo.bmp\x00" as *const u8 as *const libc::c_char,
                     raw as *const byte, nFileSize as size_t);
    if !test.is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn COM_ClearCustomizationList(mut pHead:
                                                        *mut customization_t,
                                                    mut bCleanDecals:
                                                        qboolean) {
    let mut pCurrent: *mut customization_t = 0 as *mut customization_t;
    let mut pNext: *mut customization_t = 0 as *mut customization_t;
    pCurrent = (*pHead).pNext;
    while !pCurrent.is_null() {
        pNext = (*pCurrent).pNext;
        if (*pCurrent).bInUse as libc::c_uint != 0 &&
               !(*pCurrent).pBuffer.is_null() {
            _Mem_Free((*pCurrent).pBuffer,
                      b"../engine/common/custom.c\x00" as *const u8 as
                          *const libc::c_char, 44 as libc::c_int);
        }
        if (*pCurrent).bInUse as libc::c_uint != 0 &&
               !(*pCurrent).pInfo.is_null() {
            if (*pCurrent).resource.type_0 as libc::c_uint ==
                   t_decal as libc::c_int as libc::c_uint {
                if bCleanDecals as libc::c_uint != 0 && CL_Active() != 0 {
                    ref_0.dllFuncs.R_DecalRemoveAll.expect("non-null function pointer")((*pCurrent).nUserData1);
                }
            }
            FS_FreeImage((*pCurrent).pInfo as *mut rgbdata_t);
        }
        _Mem_Free(pCurrent as *mut libc::c_void,
                  b"../engine/common/custom.c\x00" as *const u8 as
                      *const libc::c_char, 58 as libc::c_int);
        pCurrent = pNext
    }
    (*pHead).pNext = 0 as *mut customization_s;
}
#[no_mangle]
pub unsafe extern "C" fn COM_CreateCustomization(mut pListHead:
                                                     *mut customization_t,
                                                 mut pResource:
                                                     *mut resource_t,
                                                 mut playernumber:
                                                 ...
                                                 mut pOut:
                                                     *mut *mut customization_t,
                                                 mut nLumps: *mut libc::c_int)
 -> qboolean {
    let mut bError: qboolean = false_0;
    let mut checksize: fs_offset_t = 0 as libc::c_int as fs_offset_t;
    let mut pCust: *mut customization_t = 0 as *mut customization_t;
    if !pOut.is_null() { *pOut = 0 as *mut customization_t }
    pCust =
        _Mem_Alloc(host.mempool,
                   ::std::mem::size_of::<customization_t>() as libc::c_ulong,
                   true_0,
                   b"../engine/common/custom.c\x00" as *const u8 as
                       *const libc::c_char, 72 as libc::c_int) as
            *mut customization_t;
    (*pCust).resource = *pResource;
    if !((*pResource).nDownloadSize <= 0 as libc::c_int) {
        (*pCust).bInUse = true_0;
        if flags & (1 as libc::c_int) << 0 as libc::c_int != 0 {
            if HPAK_GetDataPointer(b"custom.hpk\x00" as *const u8 as
                                       *const libc::c_char, pResource,
                                   &mut (*pCust).pBuffer as
                                       *mut *mut libc::c_void as
                                       *mut *mut byte, 0 as *mut libc::c_int)
                   as u64 == 0 {
                bError = true_0
            }
        } else {
            (*pCust).pBuffer =
                FS_LoadFile((*pResource).szFileName.as_mut_ptr(),
                            &mut checksize, true_0) as *mut libc::c_void;
            if checksize as libc::c_int != (*pCust).resource.nDownloadSize {
                bError = true_0
            }
        }
        if !(bError as u64 != 0) {
            if (*pCust).resource.ucFlags as libc::c_int &
                   ...
                   (*pCust).resource.type_0 as libc::c_uint ==
                       t_decal as libc::c_int as libc::c_uint {
                (*pCust).resource.playernum = playernumber as libc::c_uchar;
                if CustomDecal_Validate((*pCust).pBuffer,
                                        (*pResource).nDownloadSize) as u64 !=
                       0 {
                    if flags & (1 as libc::c_int) << 2 as libc::c_int == 0 {
                        if (*pResource).nDownloadSize >=
                            ...
                            (*pCust).bTranslated = true_0;
                            (*pCust).nUserData1 = 0 as libc::c_int;
                            (*pCust).nUserData2 = 1 as libc::c_int;
                            if flags & (1 as libc::c_int) << 1 as libc::c_int
                                   == 0 {
                                (*pCust).pInfo =
                                    FS_LoadImage(b"#logo.bmp\x00" as *const u8
                                                 ...
                                                 (*pCust).resource.nDownloadSize
                                                     as size_t) as
                                        *mut libc::c_void
                            } else { (*pCust).pInfo = 0 as *mut libc::c_void }
                            if !nLumps.is_null() {
                                *nLumps = 1 as libc::c_int
                            }
                        }
                    }
                }
            }
            if !pOut.is_null() { *pOut = pCust }
            (*pCust).pNext = (*pListHead).pNext;
            (*pListHead).pNext = pCust;
            return true_0
        }
    }
    if !(*pCust).pBuffer.is_null() {
        _Mem_Free((*pCust).pBuffer,
                  b"../engine/common/custom.c\x00" as *const u8 as
                      *const libc::c_char, 127 as libc::c_int);
    }
    if !(*pCust).pInfo.is_null() {
        _Mem_Free((*pCust).pInfo,
                  b"../engine/common/custom.c\x00" as *const u8 as
                      *const libc::c_char, 130 as libc::c_int);
    }
    _Mem_Free(pCust as *mut libc::c_void,
              b"../engine/common/custom.c\x00" as *const u8 as
                  *const libc::c_char, 131 as libc::c_int);
    return false_0;
}
#[no_mangle]
pub unsafe extern "C" fn COM_SizeofResourceList(mut pList: *mut resource_t,
                                                mut ri: *mut resourceinfo_t)
 -> libc::c_int {
    let mut nSize: libc::c_int = 0 as libc::c_int;
    let mut p: *mut resource_t = 0 as *mut resource_t;
    memset(ri as *mut libc::c_void, 0 as libc::c_int,
           ::std::mem::size_of::<resourceinfo_t>() as libc::c_ulong);
    p = (*pList).pNext;
    while p != pList {
        nSize += (*p).nDownloadSize;
        if (*p).type_0 as libc::c_uint ==
               t_model as libc::c_int as libc::c_uint &&
               (*p).nIndex == 1 as libc::c_int {
            (*ri).info[t_world as libc::c_int as usize].size +=
                (*p).nDownloadSize
        } else { (*ri).info[(*p).type_0 as usize].size += (*p).nDownloadSize }
        p = (*p).pNext
    }
    return nSize;
}
