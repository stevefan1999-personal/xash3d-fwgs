...
extern "C" {
    ...
    #[no_mangle]
    fn Matrix3x4_TransformAABB(world: *const [vec_t; 4], mins: *const vec_t,
                               maxs: *const vec_t, absmin: *mut vec_t,
                               absmax: *mut vec_t);
    #[no_mangle]
...
===============================================================================
*/
static mut lastlink: *mut *mut efrag_t =
    0 as *const *mut efrag_t as *mut *mut efrag_t;
static mut r_pefragtopnode: *mut mnode_t =
    0 as *const mnode_t as *mut mnode_t;
static mut r_emins: vec3_t = [0.; 3];
static mut r_emaxs: vec3_t = [0.; 3];
static mut r_addent: *mut cl_entity_t =
...
*/
#[no_mangle]
pub unsafe extern "C" fn R_RemoveEfrags(mut ent: *mut cl_entity_t) {
    let mut ef: *mut efrag_t = 0 as *mut efrag_t;
    let mut old: *mut efrag_t = 0 as *mut efrag_t;
    let mut walk: *mut efrag_t = 0 as *mut efrag_t;
    let mut prev: *mut *mut efrag_t = 0 as *mut *mut efrag_t;
    ef = (*ent).efrag;
    while !ef.is_null() {
        prev = &mut (*(*ef).leaf).efrags;
        loop  {
            walk = *prev;
            if walk.is_null() { break ; }
            if walk == ef {
                // remove this fragment
                *prev = (*ef).leafnext;
                break ;
            } else { prev = &mut (*walk).leafnext }
        }
        old = ef;
        ef = (*ef).entnext;
...
===================
*/
unsafe extern "C" fn R_SplitEntityOnNode(mut node: *mut mnode_t) {
    let mut ef: *mut efrag_t = 0 as *mut efrag_t;
    ...
    if (*node).contents < 0 as libc::c_int {
        if r_pefragtopnode.is_null() { r_pefragtopnode = node }
        leaf = node as *mut mleaf_t;
        // grab an efrag off the free list
        ef = clgame.free_efrags;
        if ef.is_null() {
            Con_Printf(b"^1Error:^7 too many efrags!\n\x00" as *const u8 as
                           *const libc::c_char);
            return
            // no free fragments...
        }
        ...
        // add the entity link
        *lastlink = ef;
        lastlink = &mut (*ef).entnext;
        (*ef).entnext = 0 as *mut efrag_s;
        // set the leaf links
        (*ef).leaf = leaf;
    ...
    sides =
        if ((*(*node).plane).type_0 as libc::c_int) < 3 as libc::c_int {
            if (*(*node).plane).dist <=
                   r_emins[(*(*node).plane).type_0 as usize] {
                1 as libc::c_int
            } else if (*(*node).plane).dist >=
                          r_emaxs[(*(*node).plane).type_0 as usize] {
                2 as libc::c_int
            } else { 3 as libc::c_int }
        } else {
            BoxOnPlaneSide(r_emins.as_mut_ptr() as *const vec_t,
                           r_emaxs.as_mut_ptr() as *const vec_t,
                           (*node).plane)
        };
    ...
    if sides & 1 as libc::c_int != 0 {
        R_SplitEntityOnNode((*node).children[0 as libc::c_int as usize]);
    }
    if sides & 2 as libc::c_int != 0 {
        R_SplitEntityOnNode((*node).children[1 as libc::c_int as usize]);
    };
}
/*
===========
...
*/
#[no_mangle]
pub unsafe extern "C" fn R_AddEfrags(mut ent: *mut cl_entity_t) {
    let mut transform: matrix3x4 = [[0.; 4]; 3];
    let mut outmins: vec3_t = [0.; 3];
    let mut outmaxs: vec3_t = [0.; 3];
    let mut i: libc::c_int = 0;
    if (*ent).model.is_null() { return }
    r_addent = ent;
    lastlink = &mut (*ent).efrag;
    r_pefragtopnode = 0 as *mut mnode_t;
    // handle entity rotation for right bbox expanding
    Matrix3x4_CreateFromEntity(transform.as_mut_ptr(),
                               (*ent).angles.as_mut_ptr() as *const vec_t,
                               vec3_origin.as_mut_ptr() as *const vec_t,
                               1.0f32);
    Matrix3x4_TransformAABB(transform.as_mut_ptr() as *const [vec_t; 4],
                            (*(*ent).model).mins.as_mut_ptr() as *const vec_t,
                            (*(*ent).model).maxs.as_mut_ptr() as *const vec_t,
                            outmins.as_mut_ptr(), outmaxs.as_mut_ptr());
    i = 0 as libc::c_int;
    while i < 3 as libc::c_int {
        r_emins[i as usize] = (*ent).origin[i as usize] + outmins[i as usize];
        r_emaxs[i as usize] = (*ent).origin[i as usize] + outmaxs[i as usize];
        i += 1
    }
...
*/
#[no_mangle]
pub unsafe extern "C" fn R_StoreEfrags(mut ppefrag: *mut *mut efrag_t,
                                       mut framecount: libc::c_int) {
    let mut pent: *mut cl_entity_t = 0 as *mut cl_entity_t;
    let mut clmodel: *mut model_t = 0 as *mut model_t;
    let mut pefrag: *mut efrag_t = 0 as *mut efrag_t;
    loop  {
        pefrag = *ppefrag;
        if pefrag.is_null() { break ; }
        pent = (*pefrag).entity;
        clmodel = (*pent).model;
        match (*clmodel).type_0 as libc::c_int {
            2 | 0 | 3 | 1 => {
                pent = (*pefrag).entity;
                if (*pent).visframe != framecount {
                    if CL_AddVisibleEntity(pent, 4 as libc::c_int) as u64 != 0
                       {
