...
extern "C" {
    ...
    #[no_mangle]
    fn Image_PaletteHueReplace(palSrc: *mut byte, newHue: libc::c_int,
                               start: libc::c_int, end: libc::c_int,
                               pal_size: libc::c_int);
    #[no_mangle]
...
*/
#[no_mangle]
pub unsafe extern "C" fn CL_GetRemapInfoForEntity(mut e: *mut cl_entity_t)
 -> *mut remap_info_t {
    if e.is_null() { return 0 as *mut remap_info_t }
    if e == &mut clgame.viewent as *mut cl_entity_t {
        return *clgame.remap_info.offset(clgame.maxEntities as isize)
    }
    return *clgame.remap_info.offset((*e).curstate.number as isize);
}
/*
====================
...
*/
#[no_mangle]
pub unsafe extern "C" fn CL_CmpStudioTextures(mut numtexs: libc::c_int,
                                              mut p1: *mut mstudiotexture_t,
                                              mut p2: *mut mstudiotexture_t)
 -> qboolean {
    let mut i: libc::c_int = 0; // colormaps always has different indexes
    if p1.is_null() || p2.is_null() { return false_0 }
    i = 0 as libc::c_int;
    while i < numtexs {
        if !((*p1).flags & (1 as libc::c_uint) << 30 as libc::c_int != 0) {
            if (*p1).index != (*p2).index { return false_0 }
        }
        i += 1;
        p1 = p1.offset(1);
        p2 = p2.offset(1)
...
*/
#[no_mangle]
pub unsafe extern "C" fn CL_CreateRawTextureFromPixels(mut tx: *mut texture_t,
                                                       mut size: *mut size_t,
                                                       mut topcolor:
                                                           libc::c_int,
                                                       mut bottomcolor:
                                                           libc::c_int)
 -> *mut byte {
    static mut pin: mstudiotexture_t =
        mstudiotexture_t{name: [0; 64],
                         flags: 0,
                         width: 0,
                         height: 0,
                         index: 0,};
    ...
    *size =
        (::std::mem::size_of::<mstudiotexture_t>() as
             libc::c_ulong).wrapping_add((*tx).width.wrapping_mul((*tx).height)
                                             as
                                             libc::c_ulong).wrapping_add(768
                                                                             ...
                                                                             libc::c_ulong);
    // fill header
    if pin.name[0 as libc::c_int as usize] == 0 {
        Q_strncpy(pin.name.as_mut_ptr(),
                  b"#raw_remap_image.mdl\x00" as *const u8 as
                      *const libc::c_char,
                  ::std::mem::size_of::<[libc::c_char; 64]>() as
                      libc::c_ulong); // just in case :-)
    }
    pin.flags = (1 as libc::c_uint) << 30 as libc::c_int;
    //pin.index = (int)(tx + 1); // pointer to pixels
	// no more pointer-to-int-to-pointer casts
    Image_SetMDLPointer(tx.offset(1 as libc::c_int as isize) as *mut byte);
    pin.width = (*tx).width as int32_t;
    pin.height = (*tx).height as int32_t;
    // update palette
    pal =
        (tx.offset(1 as libc::c_int as isize) as
             *mut byte).offset((*tx).width.wrapping_mul((*tx).height) as
                                   isize);
    Image_PaletteHueReplace(pal, topcolor, (*tx).anim_min, (*tx).anim_max,
                            3 as libc::c_int);
    Image_PaletteHueReplace(pal, bottomcolor,
                            (*tx).anim_max + 1 as libc::c_int,
                            (*tx).anim_total, 3 as libc::c_int);
    return &mut pin as *mut mstudiotexture_t as *mut byte;
}
/*
====================
CL_DuplicateTexture
...
*/
#[no_mangle]
pub unsafe extern "C" fn CL_DuplicateTexture(mut entity: *mut cl_entity_t,
    ...
    let mut paletteBackup: [byte; 768] = [0; 768];
    let mut raw: *mut byte = 0 as *mut byte;
    let mut pal: *mut byte = 0 as *mut byte;
    // save off the real texture index
    index = (*ptexture).index;
    name =
        ref_0.dllFuncs.GL_TextureName.expect("non-null function pointer")(index
                                                                              as
                                                                              libc::c_uint);
    Q_snprintf(texname.as_mut_ptr(),
               ::std::mem::size_of::<[libc::c_char; 128]>() as libc::c_ulong,
               b"#%i_%s\x00" as *const u8 as *const libc::c_char,
               (*entity).curstate.number,
               name.offset(1 as libc::c_int as isize));
    // search for pixels
    i = 0 as libc::c_int;
    while i < (*model).numtextures {
        tx = *(*model).textures.offset(i as isize);
        if (*tx).gl_texturenum == index { break ; }
        i += 1
        // found
    }
    ...
    pal =
        (tx.offset(1 as libc::c_int as isize) as
             *mut byte).offset((*tx).width.wrapping_mul((*tx).height) as
                                   isize); // do copy
    memcpy(paletteBackup.as_mut_ptr() as *mut libc::c_void,
           pal as *const libc::c_void, 768 as libc::c_int as libc::c_ulong);
    raw = CL_CreateRawTextureFromPixels(tx, &mut size, topcolor, bottomcolor);
    (*ptexture).index =
    ...
    memcpy(pal as *mut libc::c_void,
           paletteBackup.as_mut_ptr() as *const libc::c_void,
...
*/
#[no_mangle]
pub unsafe extern "C" fn CL_UpdateStudioTexture(mut entity: *mut cl_entity_t,
                                                mut ptexture:
                                                    *mut mstudiotexture_t,
                                                mut topcolor: libc::c_int,
                                                mut bottomcolor:
    ...
    let mut origtexname: *const libc::c_char = 0 as *const libc::c_char;
    let mut i: libc::c_int = 0;
    let mut index: libc::c_int = 0;
    let mut size: size_t = 0;
    let mut paletteBackup: [byte; 768] = [0; 768];
    ...
    // save off the real texture index
    origtexname =
        ref_0.dllFuncs.GL_TextureName.expect("non-null function pointer")((*ptexture).index
                                                                              as
                                                                              libc::c_uint);
    // build name of original texture
    Q_strncpy(mdlname.as_mut_ptr(), (*(*entity).model).name.as_mut_ptr(),
              ::std::mem::size_of::<[libc::c_char; 128]>() as
                  libc::c_ulong); // couldn't find texture
    ...
    index =
        ref_0.dllFuncs.GL_FindTexture.expect("non-null function pointer")(texname.as_mut_ptr());
    if index == 0 { return }
    // search for pixels
    i = 0 as libc::c_int;
    while i < (*(*entity).model).numtextures {
        tx = *(*(*entity).model).textures.offset(i as isize);
        if (*tx).gl_texturenum == index { break ; }
        i += 1
        // found
    }
    ...
    if pic.is_null() {
        Con_DPrintf(b"^1Error:^7 Couldn\'t update texture %s\n\x00" as
                        *const u8 as *const libc::c_char, origtexname);
        return
    }
    index =
        ref_0.dllFuncs.GL_LoadTextureFromBuffer.expect("non-null function pointer")(origtexname,
                                                                                    pic,
                                                                                    TF_COLORMAP,
                                                                                    true_0);
    FS_FreeImage(pic);
...
*/
#[no_mangle]
pub unsafe extern "C" fn CL_UpdateAliasTexture(mut entity: *mut cl_entity_t,
                                               mut texture:
                                                   *mut libc::c_ushort,
                                               mut skinnum: libc::c_int,
                                               mut topcolor: libc::c_int,
                                               mut bottomcolor: libc::c_int) {
    let mut texname: [libc::c_char; 64] = [0; 64]; // no remapinfo in model
    let mut skin: rgbdata_t =
        rgbdata_t{width: 0,
                  height: 0,
                  depth: 0,
                  type_0: 0,
                  ...
                  fogParams: [0; 4],
                  size: 0,}; // missing texture ?
    let mut pic: *mut rgbdata_t =
        0 as
            *mut rgbdata_t; // because GL_LoadTextureInternal will freed a rgbdata_t at end
    let mut tx: *mut texture_t = 0 as *mut texture_t;
    if texture.is_null() || (*(*entity).model).textures.is_null() { return }
    tx = *(*(*entity).model).textures.offset(skinnum as isize);
    if tx.is_null() { return }
    if *texture as libc::c_int == 0 as libc::c_int {
        Q_snprintf(texname.as_mut_ptr(),
                   ::std::mem::size_of::<[libc::c_char; 64]>() as
                       libc::c_ulong,
                   b"%s:remap%i_%i\x00" as *const u8 as *const libc::c_char,
                   (*(*entity).model).name.as_mut_ptr(), skinnum,
                   (*entity).index);
        skin.width = (*tx).width as word;
        skin.height = (*tx).height as word;
        skin.numMips = 1 as libc::c_int as byte;
        skin.depth = skin.numMips as word;
        skin.size = (*tx).width.wrapping_mul((*tx).height) as size_t;
        skin.type_0 = PF_INDEXED_24 as libc::c_int as uint;
        skin.flags =
            (IMAGE_HAS_COLOR as libc::c_int | IMAGE_QUAKEPAL as libc::c_int)
                as uint;
        skin.encode = 0 as libc::c_int as word;
        skin.buffer = tx.offset(1 as libc::c_int as isize) as *mut byte;
        skin.palette = skin.buffer.offset(skin.size as isize);
        pic = FS_CopyImage(&mut skin);
        *texture =
            ref_0.dllFuncs.GL_LoadTextureFromBuffer.expect("non-null function pointer")(texname.as_mut_ptr(),
                                                                                        pic,
                                                                                        TF_KEEP_SOURCE,
                                                                                        false_0)
                as libc::c_ushort
    ...
    ref_0.dllFuncs.GL_ProcessTexture.expect("non-null function pointer")(*texture
                                                                             as
                                                                             libc::c_int,
                                                                         -1.0f32,
                                                                         topcolor,
...
*/
#[no_mangle]
pub unsafe extern "C" fn CL_AllocRemapInfo(mut entity: *mut cl_entity_t,
                                           mut model: *mut model_t,
                                           mut topcolor: libc::c_int,
                                           mut bottomcolor: libc::c_int) {
    let mut info: *mut remap_info_t = 0 as *mut remap_info_t;
    let mut phdr: *mut studiohdr_t = 0 as *mut studiohdr_t;
    let mut ahdr: *mut aliashdr_t = 0 as *mut aliashdr_t;
    let mut src: *mut mstudiotexture_t = 0 as *mut mstudiotexture_t;
    let mut dst: *mut mstudiotexture_t = 0 as *mut mstudiotexture_t;
    let mut i: libc::c_int = 0;
    let mut size: libc::c_int = 0;
    if entity.is_null() { return }
    i =
        if entity == &mut clgame.viewent as *mut cl_entity_t {
            clgame.maxEntities
        } else { (*entity).curstate.number };
    if model.is_null() ||
           (*model).type_0 as libc::c_int != mod_alias as libc::c_int &&
               (*model).type_0 as libc::c_int != mod_studio as libc::c_int {
        // entity has changed model by another type, release remap info
        if !(*clgame.remap_info.offset(i as isize)).is_null() {
            CL_FreeRemapInfo(*clgame.remap_info.offset(i as isize));
            let ref mut fresh0 = *clgame.remap_info.offset(i as isize);
            *fresh0 = 0 as *mut remap_info_t
        }
        return
        // missed or hide model, ignore it
    }
    // model doesn't contains remap textures
    if (*model).numtextures <= 0 as libc::c_int {
        // entity has changed model with no remap textures
        if !(*clgame.remap_info.offset(i as isize)).is_null() {
            CL_FreeRemapInfo(*clgame.remap_info.offset(i as
                                                           isize)); // bad model?
            let ref mut fresh1 = *clgame.remap_info.offset(i as isize);
            *fresh1 = 0 as *mut remap_info_t
        }
        return
    }
    if (*model).type_0 as libc::c_int == mod_studio as libc::c_int {
        phdr = Mod_StudioExtradata(model) as *mut studiohdr_t;
        ...
        dst =
            if !(*clgame.remap_info.offset(i as isize)).is_null() {
                (**clgame.remap_info.offset(i as isize)).ptexture
            } else { 0 as *mut mstudiotex_s };
        // NOTE: we must copy all the structures 'mstudiotexture_t' for easy access when model is rendering
        if CL_CmpStudioTextures((*phdr).numtextures, src, dst) as u64 == 0 ||
               (**clgame.remap_info.offset(i as isize)).model != model {
            ...
            size =
                (::std::mem::size_of::<remap_info_t>() as
                     libc::c_ulong).wrapping_add((::std::mem::size_of::<mstudiotexture_t>()
                                                      as
                                                      libc::c_ulong).wrapping_mul((*phdr).numtextures
                                                                                      as
                                                                                      libc::c_ulong))
                    as libc::c_int;
            let ref mut fresh2 = *clgame.remap_info.offset(i as isize);
            *fresh2 =
                _Mem_Alloc(clgame.mempool, size as size_t, true_0,
                           b"../engine/client/cl_remap.c\x00" as *const u8 as
                               *const libc::c_char, 294 as libc::c_int) as
                    *mut remap_info_t;
            info = *fresh2;
            (*info).ptexture =
                info.offset(1 as libc::c_int as isize) as
                    *mut mstudiotexture_t
            // textures are immediately comes after remap_info
        } else {
            // studiomodel is valid, nothing to change
            return
        }
        (*info).numtextures = (*phdr).numtextures as libc::c_short;
        (*info).topcolor = topcolor as libc::c_short;
        (*info).bottomcolor = bottomcolor as libc::c_short;
        src =
            (phdr as *mut byte).offset((*phdr).textureindex as isize) as
                *mut mstudiotexture_t;
        dst = (*info).ptexture;
        ...
        memcpy(dst as *mut libc::c_void, src as *const libc::c_void,
               (::std::mem::size_of::<mstudiotexture_t>() as
                    libc::c_ulong).wrapping_mul((*phdr).numtextures as
                                                    libc::c_ulong));
        // make local copies for remap textures
        i = 0 as libc::c_int; // bad model?
        while i < (*info).numtextures as libc::c_int {
            if (*dst.offset(i as isize)).flags &
                   (1 as libc::c_uint) << 30 as libc::c_int != 0 {
                CL_DuplicateTexture(entity, model,
                                    &mut *dst.offset(i as isize), topcolor,
                                    bottomcolor);
            }
            i += 1
        }
    } else if (*model).type_0 as libc::c_int == mod_alias as libc::c_int {
        ahdr = Mod_AliasExtradata(model) as *mut aliashdr_t;
        if ahdr.is_null() { return }
        // NOTE: we must copy all the structures 'mstudiotexture_t' for easy access when model is rendering
        if (*clgame.remap_info.offset(i as isize)).is_null() ||
               (**clgame.remap_info.offset(i as isize)).model != model {
            // this code catches studiomodel change with another studiomodel with remap textures
            ...
            let ref mut fresh3 = *clgame.remap_info.offset(i as isize);
            *fresh3 =
                _Mem_Alloc(clgame.mempool,
                           ::std::mem::size_of::<remap_info_t>() as
                               libc::c_ulong, true_0,
                           b"../engine/client/cl_remap.c\x00" as *const u8 as
                               *const libc::c_char, 331 as libc::c_int) as
                    *mut remap_info_t;
            info = *fresh3
        } else {
        ...
        (*info).numtextures = (*model).numtextures as libc::c_short;
...
*/
#[no_mangle]
pub unsafe extern "C" fn CL_UpdateRemapInfo(mut entity: *mut cl_entity_t,
                                            mut topcolor: libc::c_int,
                                            mut bottomcolor: libc::c_int) {
    let mut info: *mut remap_info_t = 0 as *mut remap_info_t; // no remap info
    let mut i: libc::c_int = 0; // values is valid
    i =
        if entity == &mut clgame.viewent as *mut cl_entity_t {
            clgame.maxEntities
        } else { (*entity).curstate.number };
    info = *clgame.remap_info.offset(i as isize);
    if info.is_null() { return }
    if (*info).topcolor as libc::c_int == topcolor &&
           (*info).bottomcolor as libc::c_int == bottomcolor {
        return
    }
    i = 0 as libc::c_int;
    while i < (*info).numtextures as libc::c_int {
        if !(*info).ptexture.is_null() {
            if (*(*info).ptexture.offset(i as isize)).flags &
                   (1 as libc::c_uint) << 30 as libc::c_int != 0 {
                CL_UpdateStudioTexture(entity,
                                       &mut *(*info).ptexture.offset(i as
                                                                         isize),
                                       topcolor, bottomcolor);
            }
        } else {
            CL_UpdateAliasTexture(entity,
                                  &mut *(*info).textures.as_mut_ptr().offset(i
                                                                                 as
                                                                                 isize),
                                  i, topcolor, bottomcolor);
        }
        i += 1
    }
    (*info).topcolor = topcolor as libc::c_short;
...
*/
#[no_mangle]
pub unsafe extern "C" fn CL_FreeRemapInfo(mut info: *mut remap_info_t) {
    let mut i: libc::c_int = 0;
    ...
    while i < (*info).numtextures as libc::c_int {
        if !(*info).ptexture.is_null() {
            if (*(*info).ptexture.offset(i as isize)).flags &
                   (1 as libc::c_uint) << 30 as libc::c_int != 0 {
                ref_0.dllFuncs.GL_FreeTexture.expect("non-null function pointer")((*(*info).ptexture.offset(i
                                                                                                                as
                                                                                                                isize)).index
                                                                                      as
                                                                                      libc::c_uint);
        ...
        if (*info).textures[i as usize] as libc::c_int != 0 as libc::c_int {
            ref_0.dllFuncs.GL_FreeTexture.expect("non-null function pointer")((*info).textures[i
                                                                                                   as
                                                                                                   usize]
                                                                                  as
                                                                                  libc::c_uint);
    ...
    _Mem_Free(info as *mut libc::c_void,
              b"../engine/client/cl_remap.c\x00" as *const u8 as
                  *const libc::c_char, 412 as libc::c_int);
    // release struct
}
/*
====================
...
*/
#[no_mangle]
pub unsafe extern "C" fn CL_ClearAllRemaps() {
    let mut i: libc::c_int = 0;
    if !clgame.remap_info.is_null() {
        i = 0 as libc::c_int;
        while i < clgame.maxRemapInfos {
            if !(*clgame.remap_info.offset(i as isize)).is_null() {
                CL_FreeRemapInfo(*clgame.remap_info.offset(i as isize));
            }
            i += 1
        }
        _Mem_Free(clgame.remap_info as *mut libc::c_void,
                  b"../engine/client/cl_remap.c\x00" as *const u8 as
                      *const libc::c_char, 433 as libc::c_int);
    }
    clgame.remap_info = 0 as *mut *mut remap_info_t;
}
