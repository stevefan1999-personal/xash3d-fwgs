...
extern "C" {
    ...
    #[no_mangle]
    fn GL_FrustumCullBox(out: *mut gl_frustum_t, mins: *const vec_t,
                         maxs: *const vec_t, userClipFlags: libc::c_int)
     -> qboolean;
    #[no_mangle]
    fn GL_FrustumCullSphere(out: *mut gl_frustum_t, centre: *const vec_t,
     ...
     -> qboolean;
    #[no_mangle]
    fn Matrix4x4_VectorRotate(in_0: *const [vec_t; 4],
...
*/
#[no_mangle]
pub unsafe extern "C" fn R_CullBox(mut mins: *const vec_t,
                                   mut maxs: *const vec_t) -> qboolean {
    return GL_FrustumCullBox(&mut RI.frustum, mins, maxs, 0 as libc::c_int);
}
/*
=================
R_CullSphere
...
*/
#[no_mangle]
pub unsafe extern "C" fn R_CullSphere(mut centre: *const vec_t,
                                      radius: libc::c_float) -> qboolean {
    return GL_FrustumCullSphere(&mut RI.frustum, centre, radius,
                                0 as libc::c_int);
}
/*
=============
...
*/
#[no_mangle]
pub unsafe extern "C" fn R_CullModel(mut e: *mut cl_entity_t,
 ...
 -> libc::c_int {
    if e == gEngfuncs.GetViewModel.expect("non-null function pointer")() {
        ...
        if RI.params as libc::c_uint & (1 as libc::c_uint) << 0 as libc::c_int
               ...
               Some(gEngfuncs.EngineGetParm.expect("non-null function pointer")).expect("non-null function pointer")(PARM_THIRDPERSON
                                                                                                                         as
                                                                                                                         libc::c_int,
                                                                                                                     0
                                                                                                                         as
    ...
    if R_CullBox(absmin, absmax) as u64 != 0 { return 1 as libc::c_int }
    return 0 as libc::c_int;
}
/*
=================
...
*/
#[no_mangle]
pub unsafe extern "C" fn R_CullSurface(mut surf: *mut msurface_t,
                                       mut frustum: *mut gl_frustum_t,
                                       mut clipflags: uint) -> libc::c_int {
    let mut e: *mut cl_entity_t = RI.currententity;
    if surf.is_null() || (*surf).texinfo.is_null() ||
           (*(*surf).texinfo).texture.is_null() {
        return 4 as libc::c_int
    }
    if (*r_nocull).value != 0. { return 0 as libc::c_int }
    ...
    if !((*e).origin[0 as libc::c_int as usize] == 0.0f32 &&
             (*e).origin[1 as libc::c_int as usize] == 0.0f32 &&
             (*e).origin[2 as libc::c_int as usize] == 0.0f32 &&
             ((*e).angles[0 as libc::c_int as usize] == 0.0f32 &&
                  (*e).angles[1 as libc::c_int as usize] == 0.0f32 &&
                  (*e).angles[2 as libc::c_int as usize] == 0.0f32)) {
        frustum = 0 as *mut gl_frustum_t
    }
    if !((*(*surf).plane).normal[0 as libc::c_int as usize] == 0.0f32 &&
             (*(*surf).plane).normal[1 as libc::c_int as usize] == 0.0f32 &&
             (*(*surf).plane).normal[2 as libc::c_int as usize] == 0.0f32) {
        let mut dist: libc::c_float = 0.;
        // can use normal.z for world (optimisation)
        if RI.drawOrtho as u64 != 0 {
            let mut orthonormal: vec3_t = [0.; 3];
            if e ==
                   gEngfuncs.GetEntityByIndex.expect("non-null function pointer")(0
                                                                                      as
                                                                                      libc::c_int)
               {
                orthonormal[2 as libc::c_int as usize] =
                    (*(*surf).plane).normal[2 as libc::c_int as usize]
            } else {
                Matrix4x4_VectorRotate(RI.objectMatrix.as_mut_ptr() as
                                           *const [vec_t; 4],
                                       (*(*surf).plane).normal.as_mut_ptr() as
                                           *const libc::c_float,
                                       orthonormal.as_mut_ptr());
            }
            dist = orthonormal[2 as libc::c_int as usize]
        } else {
            dist =
                (if ((*(*surf).plane).type_0 as libc::c_int) <
                        3 as libc::c_int {
                     tr.modelorg[(*(*surf).plane).type_0 as usize]
                 } else {
        ...
        if glState.faceCull == 0x404 as libc::c_int {
            if (*surf).flags as libc::c_uint &
                   (1 as libc::c_uint) << 1 as libc::c_int != 0 {
                if dist >= -0.01f32 { return 1 as libc::c_int }
                // wrong side
            } else if dist <= 0.01f32 { return 1 as libc::c_int }
        } else if glState.faceCull == 0x405 as libc::c_int {
            if (*surf).flags as libc::c_uint &
                   (1 as libc::c_uint) << 1 as libc::c_int != 0 {
                if dist <= 0.01f32 { return 1 as libc::c_int }
                // wrong side
    ...
    if !frustum.is_null() &&
           GL_FrustumCullBox(frustum,
                             (*(*surf).info).mins.as_mut_ptr() as
                                 *const vec_t,
                             (*(*surf).info).maxs.as_mut_ptr() as
                                 *const vec_t, clipflags as libc::c_int) as
               libc::c_uint != 0 {
