...
#[no_mangle]
pub static mut blocklights: [libc::c_uint; 10240] = [0; 10240];
/*
=============================================================================
...
pub unsafe extern "C" fn CL_RunLightStyles() {
    let mut i: libc::c_int = 0; //r_lighting_modulate->value;
        ...
        if (*gEngfuncs.pfnGetModelByIndex.expect("non-null function pointer")(1
                                                                                  ...
                                                                                  libc::c_int)).lightdata.is_null()
           {
            ...
            if (*ls).length == 0 {
                ...
                // || !CVAR_TO_BOOL( cl_lightstyle_lerping ))
                tr.lightstylevalue[i as usize] =
                ...
                if !(ds >
                    ...
                    while map < 4 as libc::c_int &&
                        ...
                        if tr.ignore_lightgamma as u64 != 0 {
                            (*cv).r =
                                ((*cv).r as libc::c_double +
                                     ((*lm).r as
                                          libc::c_uint).wrapping_mul(scale) as
                                         libc::c_double * 2.5f64) as
                                    libc::c_uint;
                            // scale;
                            (*cv).g =
                                ((*cv).g as libc::c_double +
                                     ((*lm).g as
                                          libc::c_uint).wrapping_mul(scale) as
                                         libc::c_double * 2.5f64) as
                                    libc::c_uint; // scale;
                            (*cv).b =
                                ((*cv).b as libc::c_double +
                                     ((*lm).b as
                                          libc::c_uint).wrapping_mul(scale) as
                                         libc::c_double * 2.5f64) as
                                    libc::c_uint
                            ...
                            (*cv).r =
                                (*cv).r.wrapping_add((gEngfuncs.LightToTexGamma.expect("non-null function pointer")((*lm).r)
                                                          as
                                                          libc::c_uint).wrapping_mul(scale)); // skip to next lightmap
                            (*cv).g =
...
pub unsafe extern "C" fn R_LightVec(mut start: *const vec_t,
    ...
    if !lspot.is_null() && !lvec.is_null() {
        // CVAR_TO_BOOL( r_lighting_extended ) &&
