...
*/
#[no_mangle]
pub unsafe extern "C" fn CL_CheckFile(mut msg: *mut sizebuf_t,
                                      mut pResource: *mut resource_t)
 -> qboolean {
    let mut filepath: [libc::c_char; 64] = [0; 64];
    match (*pResource).type_0 as libc::c_uint {
        0 | 2 => {
            // built-in resources not needs to be downloaded
            if (*pResource).szFileName[0 as libc::c_int as usize] as
                   libc::c_int == '*' as i32 {
                return true_0
            }
        }
    ...
    if (*pResource).nDownloadSize == -(1 as libc::c_int) {
        (*pResource).ucFlags =
    ...
    if (*pResource).type_0 as libc::c_uint ==
        ...
        Q_strncpy(filepath.as_mut_ptr(), (*pResource).szFileName.as_mut_ptr(),
                  ::std::mem::size_of::<[libc::c_char; 64]>() as
                      libc::c_ulong);
    }
    if COM_IsSafeFileToDownload(filepath.as_mut_ptr()) as u64 == 0 {
        Con_Reportf(b"refusing to download %s\n\x00" as *const u8 as
                        *const libc::c_char, filepath.as_mut_ptr());
        return true_0
    }
    if cl_allow_download.value == 0. {
        Con_Reportf(b"Download refused, cl_allow_download is 0\n\x00" as
                        *const u8 as *const libc::c_char);
        return true_0
    }
    if cls.state as libc::c_uint == ca_active as libc::c_int as libc::c_uint
           && cl_download_ingame.value == 0. {
        Con_Reportf(b"In-game download refused...\n\x00" as *const u8 as
                        *const libc::c_char);
        return true_0
    }
    // don't request downloads from local client it's silly
    if Host_IsLocalClient() as libc::c_uint != 0 ||
           FS_FileExists(filepath.as_mut_ptr(), false_0 as libc::c_int) != 0 {
        return true_0
    }
    if cls.demoplayback != 0 {
        Con_Reportf(b"^3Warning:^7 file %s missing during demo playback.\n\x00"
                        as *const u8 as *const libc::c_char,
                    filepath.as_mut_ptr());
        return true_0
    }
    if cl.downloadUrl[0 as libc::c_int as usize] != 0 {
        HTTP_AddDownload(filepath.as_mut_ptr(), (*pResource).nDownloadSize,
                         true_0);
    ...
    MSG_WriteString(msg,
                    va(b"dlfile %s\x00" as *const u8 as *const libc::c_char,
                       filepath.as_mut_ptr()));
    host.downloadcount += 1;
    return false_0;
}
#[no_mangle]
pub unsafe extern "C" fn CL_AddToResourceList(mut pResource: *mut resource_t,
    ...
    if (*pList).pPrev.is_null() || (*pList).pNext.is_null() {
        Host_Error(b"Resource list corrupted.\n\x00" as *const u8 as
                       *const libc::c_char);
    }
...
#[no_mangle]
pub unsafe extern "C" fn CL_RemoveFromResourceList(mut pResource:
                                                       *mut resource_t) {
    if (*pResource).pPrev.is_null() || (*pResource).pNext.is_null() {
        Host_Error(b"mislinked resource in CL_RemoveFromResourceList\n\x00" as
                       *const u8 as *const libc::c_char);
    }
    if (*pResource).pNext == pResource || (*pResource).pPrev == pResource {
        Host_Error(b"attempt to free last entry in list.\n\x00" as *const u8
                       as *const libc::c_char);
    }
...
#[no_mangle]
pub unsafe extern "C" fn CL_MoveToOnHandList(mut pResource: *mut resource_t) {
    if pResource.is_null() {
        Con_Reportf(b"Null resource passed to CL_MoveToOnHandList\n\x00" as
                        *const u8 as *const libc::c_char);
        return
    }
    CL_RemoveFromResourceList(pResource);
    CL_AddToResourceList(pResource, &mut cl.resourcesonhand);
}
#[no_mangle]
pub unsafe extern "C" fn CL_ClearResourceList(mut pList: *mut resource_t) {
    let mut p: *mut resource_t = 0 as *mut resource_t;
    let mut n: *mut resource_t = 0 as *mut resource_t;
    p = (*pList).pNext;
    while p != pList && !p.is_null() {
        n = (*p).pNext;
        CL_RemoveFromResourceList(p);
        _Mem_Free(p as *mut libc::c_void,
                  b"../engine/client/cl_custom.c\x00" as *const u8 as
...
#[no_mangle]
pub unsafe extern "C" fn CL_ClearResourceLists() {
    CL_ClearResourceList(&mut cl.resourcesneeded);
    CL_ClearResourceList(&mut cl.resourcesonhand);
}
