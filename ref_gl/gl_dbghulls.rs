...
extern "C" {
    ...
    #[no_mangle]
    static mut r_showhull: *mut cvar_t;
    #[no_mangle]
...
#[no_mangle]
pub unsafe extern "C" fn R_DrawWorldHull() {
    let mut hull: *mut hull_model_t =
        &mut *(*gEngfuncs.GetWorld.expect("non-null function pointer")()).hull_models.offset(0
                                                                                                 as
                                                                                                 libc::c_int
                                                                                                 as
                                                                                                 isize)
            as *mut hull_model_t;
    let mut poly: *mut winding_t = 0 as *mut winding_t;
    let mut i: libc::c_int = 0;
    if (*r_showhull).flags & (1 as libc::c_int) << 13 as libc::c_int != 0 {
        let mut val: libc::c_int =
            if (*r_showhull).value as libc::c_int >= 0 as libc::c_int {
                if ((*r_showhull).value as libc::c_int) < 3 as libc::c_int {
                    (*r_showhull).value as libc::c_int
                } else { 3 as libc::c_int }
            } else { 0 as libc::c_int };
        if val != 0 {
            gEngfuncs.Mod_CreatePolygonsForHull.expect("non-null function pointer")(val);
        }
        (*r_showhull).flags =
            (*r_showhull).flags & !((1 as libc::c_int) << 13 as libc::c_int)
    }
    if if !r_showhull.is_null() && (*r_showhull).value != 0.0f32 {
    ...
    while &mut (*poly).chain as *mut hullnode_t !=
              &mut (*hull).polys as *mut hullnode_t {
        srand(poly as libc::c_uint);
        pglColor3f.expect("non-null function pointer")(((rand() %
                                                             256 as
                                                                 libc::c_int)
                                                            as libc::c_double
                                                            / 255.0f64) as
        ...
        while i < (*poly).numpoints {
            pglVertex3fv.expect("non-null function pointer")((*poly).p[i as
                                                                           usize].as_mut_ptr());
            i += 1
        }
...
#[no_mangle]
pub unsafe extern "C" fn R_DrawModelHull() {
    let mut hull: *mut hull_model_t = 0 as *mut hull_model_t;
    let mut poly: *mut winding_t = 0 as *mut winding_t;
    let mut i: libc::c_int = 0;
    if if !r_showhull.is_null() && (*r_showhull).value != 0.0f32 {
           true_0 as libc::c_int
    ...
    if RI.currentmodel.is_null() ||
           (*RI.currentmodel).name[0 as libc::c_int as usize] as libc::c_int
               != '*' as i32 {
        return
    }
    i =
        atoi((*RI.currentmodel).name.as_mut_ptr().offset(1 as libc::c_int as
                                                             isize));
    if i < 1 as libc::c_int ||
           i >=
               (*gEngfuncs.GetWorld.expect("non-null function pointer")()).num_hull_models
       {
        return
    }
    hull =
        &mut *(*gEngfuncs.GetWorld.expect("non-null function pointer")()).hull_models.offset(i
                                                                                                 as
                                                                                                 isize)
            as *mut hull_model_t;
